...
output:
  xaringan::moon_reader:
    includes:
      after_body: ['mylibs/collapseoutput.js','mylibs/hideOutput.js', 'mylibs/zoom.html', 'mylibs/manualcollapseinput.html']
    transition: slide
    lib_dir: libs
...
- Pacientes cursando tratamiento por consumo problemático sustancias (2010-2019)

- Usuarios pueden haber experimentado el evento más de una vez, en distintas oportunidades

- Ej. Paciente N°55 completa tto. residencial y experimenta el evento de interés (readmisión)

- Completar el tratamiento podría modificar 
...
- El diseño y la estrategia analítica hasta el momento no permite responder a una pregunta causal de manera adecuada
...
- **Eventuales competidores**: tiempo en tratamiento (especialmente residenciales) o una sentencia **aunque debemos ver si es posible conocer este tiempo de censura**, muerte
...
- Explicar causal approach, contrafactual, ajuste por confusión, hay métodos que restringen y otros que estandarizan
...
- El control de confusión es una forma de cerrar puertas traseras, siempre y cuando se cumplan otros supuestos (no hay error de medición, variables no-observadas (confusión residual), mediadores afectad)
...
- The conditional approach handles confounders using stratification or modeling (e.g., adding covariates to be regressed to the outcome).
...
- dt1 = `Meses libre de readmisión Primer tto.`           ∼ Poisson(λ=4)  
- dt2 = `Meses libre de readmisión Segundo tto.`          ∼ Poisson(λ=3)
- dt3 = `Meses libre de readmisión Tercer tto.`           ∼ Poisson(λ=2)  
- v1 = `Confusor tiempo-dependiente (t1)`                 ∼ $5+ 0.4\beta_{Tratamiento\,residencial\,basal} +0.78\beta_{Meses\,libre\,de\,readmision\,1}+ N(0, \sqrt{0.99})$
- v2 = `Confusor tiempo-dependiente (t2)`                 ∼ $5+ 0.4\beta_{Tratamiento\,residencial\,(2^{do})} +0.78\beta_{Meses\,libre\,de\,readmision\,1}+ N(0, \sqrt{0.55})$
- v3 = `Confusor tiempo-dependiente (t3)`                 ∼ $5+ 0.4\beta_{Tratamiento\,residencial\,(3^{er})} +0.78\beta_{Meses\,libre\,de\,readmision\,1}+ N(0, \sqrt{0.33})$ `
...
#Primeros 4 filas
head(datos,4) %>% 
  knitr::kable("markdown", caption="Muestra de la base de datos (Primeras 4 filas)") %>%
...
- Se asume que todos cursaron el tratamiento y no hay censura, que la velocidad es constante y los riesgos también.
- Asumiendo que los riesgos son constantes y proporcionales (Pierce)
- More generally, assuming constant hazard rates over fixed time intervals (known as a piecewise-exponential model) you can fit fairly flexible survival models in the form of poisson GLM
...
ggdag:::if_not_tidy_daggity(tidy_dag34) %>% ggdag:::node_status() %>% 
  ...
  scale_adjusted()+ ggdag:::breaks(c("exposure", "outcome","latent"))+
      theme_dag()+
  geom_dag_edges_arc(curvature = c(rep(0,13),5,0,5,rep(0,9)))+ #14 y 16 de 24
   ggdag:::geom_dag_point(size = 16)+
  ggdag:::geom_dag_label_repel(ggplot2::aes_string(label = "label", 
            fill = "status"), size = 4.88, col = "white", 
            show.legend = FALSE)+
...
- Se estima una ponderación inversa al tratamiento que tiene en cuenta exposición y/o confusores tiempo-dependiente $`r Cite(myBib, c("ipw2011"))`$.
