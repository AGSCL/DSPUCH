...
1.	Mi interés principalmente proviene de una investigación en la que estoy participando, en que analizo trayectorias de tratamientos de drogas del 2010-2019. En esta imagen sólo expongo una muestra con una estructura hipotética
Cada fila es un paciente, una línea es un tratamiento, el espacio blanco entre ellos es el tiempo en que no están en tratamientos. Los puntos son completar el tratamiento y las líneas amarillas es cursar un tratamiento residencial. Por ejemplo, en este caso, el Paciente N°55 completa tto. residencial y experimenta el evento de interés (readmisión), en ambos completa el tratamiento.
2.	El objetivo de ese estudio era estimar ese efecto mediante 4 regresiones secuenciales y estratificado por la presencia de cada evento (ej., se partió con 22.000 usuarios que sólo tenían una readmisión; y ya a la 4ta se estimó el efecto sólo con 200 usuarios). Como había una razonable sospecha de dependencia en tiempos anteriores, se generó un modelo semi-markov. Al margen de eso, hay varios problemas.
3.	El modelo causal es difícil de representar para mí. Sobre todo porque a la base se generó un pareamiento. Hernan señala que no es posible representar en un diagrama un pareamiento (Al margen de eso, ya también es discutible su implicancia causal, ya que agrupa al 90% de los tratamientos residenciales, pero al 18% de los ambulatorios. Es un efecto en los tratados. Se puede observar que hay muchos colliders (en LM) y puertas traseras no cerradas ().The total effect cannot be estimated due to adjustment for an intermediate or a descendant of an intermediate.).
4.	Robins 1986 en su momento estableció un gráfico bastante similar, en que  L=0 y L=1 son los resultados del tratamiento (completa/no-completa), y los A son residencial/ambulatorio. Y= es readmisión  POR LO VISTO, SON EFECTOS ANIDADOS PRODUCTO DE LA COMBINACIÓN DE CADA CONFIGURACION (EJ., QUIEN PRESENTA UN REGIMEN EN QUE COMPLETA UN RESIDENCIAL A LA BASE, SEGUIDO DE COMPLETAR EL SEGUNDO TRATAMIENTO RESIDENCIAL,VA A TENER UN RIESGO DE READMISIÓN DISTINTO  DE AHÍ QUE EL EFECTO ES CONDICIONAL A ESTE REGIMEN DE EXPOSICION Y MEDIACION
...
2- **Causalidad**: La tarea de la epidemiología, definición de causalidad y de confusión. Distribución marginal vs. conjunta, etc. Datos observacionales. Supuestos: SUTVA; Sesgos.Supuestos a la base, amenazas, sesgos, y particularmente sesgos que ocurren para este diseño de datos.
-	Muchas preguntas en salud pública se remiten a cuestiones epidemiológicas. Desde determinantes (ej., vulnerabilidad, PP PP), pasando por factores de riesgo, procesos biológicos/mecanismos etiológicos, etc.
...
o	Hay asociación cuando el tener información sobre una variable me permite predecir mejor en promedio el resultado de Y, incluso si esa variable no causa Y. El mecanismo no lo sabemos.
o	Uno de los desafíos es poder determinar si por sí sola una variable X causa Y
-	Las preguntas epidemiológicas no se limitan a describir sino que algunas apuntan a un análisis causal (Kaufman Santiago 2020 Lunes). Al margen de la tarea valorable de la vigilancia, la predicción en el futuro, el “Mundo hipotético”, ¿qué pasaría si hacemos/aplicamos X? (efectos de causas)/ qué causa Y (causas de efectos)” (Gelman2013)
-	Nos restringiremos a al estudio de las intervenciones. [Tener presente los alcances del estudio de potential outcomes framework]
-	En un mundo complejo como el de hoy, es necesario observar los fenómenos de manera multivariable, razonamiento estadístico, el efecto de intervenir sobre una variable sobre otra. Cummiskey2020
-	Ensayos clínicos controlados, buscan estimar el impacto de un tratamiento, patrón oro de evidencia, aleatorizados (NEFRO)
-	Incluso ellos tienen limitaciones: no son aplicables, pueden carecer de potencia estadística, pueden ser no éticos, por diseño; por eso estudios observacionales (NEFRO)
-	El control de la confusión es uno de los pasos más desafiantes en la conducción de estudios observacionales (MANSOURNIA) y a menudo puede ocurrir también en ensayos clínicos.
-	El marco de inferencia contrafactual (potential outcomes framework) (Neyman, 1923; Rubin, 1974), iniciado por Neyman y continuado por Rubin y Robins, busca comparar teóricamente qué ocurriría si todos los pacientes hubieran sido tratados frente a si ningún paciente hubiese sido tratado.
...
o	Asociaciones: correlaciones, regresiones, dependencia, independencia condicional, verosimilitud, colapsabilidad, PS, RR, OR, marginalización, condicionalización, controlando por.
o	Causal: Aleatorización, influencia, efecto, confusión, manteniendo constante, disturbance, correlación espúrea, estabilidad, IV, intervención, explicación, atribución
o	“different risk in two disjoint subsets of the population determined 12 Causal Inference by the subjects’ actual treatment value ( = 1 or  = 0), whereas causation is defined by a different risk in the entire population under two different The difference between association treatment values ( = 1 or  = 0)” (Hernan y Robins)
...
-	RCTs se puede con la aleatorización. EN estudios observacionales es aún más importante inspeccionar en qué medida se cumplen estos supuestos. stable-unit-treatment-value assumption (SUTVA)” (Rubin 1980). La validez de nuestras inferencias causales requieren dichas condiciones:
-	Nos obliga a pensar el mecanismo de asignación (assignment mechanism) para definir efectos causales para identificarlos
o	Consistencia: Si un individuo tiene una exposición A=a, entonces su resultado Y = Y(a). ¿Todos recibieron el tratamiento en medida similar? ¿Lo mismo ocurre con quienes no lo recibieron? 
...
o	Positividad: ¿La distribución de las covariables L en tratados es similar con no-tratados?, la probabilidad de asignación está entre 0 y 1, por lo que se puede tener tratados y no tratados. Cuando hay expuestos y no expuestos en todos los niveles de confusión  Esto a menudo nos e ve en las regresiones.
o	Intercambiabilidad: Debiese ser la primera, pero la dejé al final por ser la más importante. Fuera de contextos aleatorios, la intercambiabilidad será condicional a los confusores ¿Son comparables los grupos, dada la confusión?
...
- Lo anterior, aplicando un modelo de ponderación inversa de la probabilidad de asignación de tratamiento , y luego un modelo marginal estructural en base a un modelo - GEE con errores estándar robustos  poisson, con estructura de correlación autorregresiva 1 para datos anidados por paciente (para reflejar la dependencia en los resultados del tratamiento anterior).
