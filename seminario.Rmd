...
ggdag:::if_not_tidy_daggity(tidy_dag23) %>% ggdag:::node_status() %>% 
  ggplot2::ggplot(ggplot2::aes(x = x, y = y, xend = xend, 
                                 yend = yend, color = status, shape=factor(adjusted)))+ edge_function()+scale_adjusted()+ ggdag:::breaks(c("exposure", "outcome","latent"))+
...
Este proyecto apunta a servir como un material introductorio sobre modelos estructurales marginales para el control de sesgos en estudios observacionales con factores de riesgo y exposición tiempo-dependientes. 

--
- Se mostrará su estimación paso a paso y mediante un ejercicio reproducible. 

--
- Se utilizará un una base de datos simulada para ilustrar el proceso.
...
```{r p1-cont_conf, eval=T,  dev.args = list(bg = 'transparent'), echo=T, warning=FALSE, include=T, paged.print=TRUE, fig.align="center", fig.width = 7, fig.height = 5, out.width="70%", out.height="70%", error=T, dpi=750, fig.showtext=T}
library(tidyverse)
library(gganimate)
library(ggthemes)

df <- data.frame(L = as.integer((1:200>100))) %>%
  mutate(A = .5+2*L + rnorm(200)) %>%
...
before_cor <- as.expression(paste0("1. Datos crudos: r[total]=",round(cor(df$A,df$Y),3)))
after_cor <- as.expression(paste0(bquote(6~Analizo~lo~resultante:~correlación~entre~A~e~Y|L  r[partial]:round(cor(df$A-df$mean_A,df$Y-df$mean_Y),3))))
...
dffull <- rbind(
  ...
  df %>% mutate(A = A - mean_A,Y = Y - mean_Y,mean_A=NA,mean_Y=NA,time==parse(text = after_cor)))
...
p <- ggplot(dffull,aes(y=Y,x=A,color=as.factor(L)))+geom_point()+
  ...
  labs(caption="Nota. Fuente: Nick Huntington-Klein (https://github.com/NickCH-K/causalgraphs)")+
  labs(title = 'La relación entre X e Y Controlando por variable binaria L\n{next_state}')+
  transition_states(time,transition_length=c(12,32,12,32,12,12),state_length=c(160,100,75,100,75,160),wrap=FALSE)+
  labs(subtitle = as.expression(bquote('my text and {time}')))+
...
```{r p1-cont_conf2, eval=T,  dev.args = list(bg = 'transparent'), echo=T, warning=FALSE, include=T, paged.print=TRUE, fig.align="center", fig.width = 7, fig.height = 5, out.width="70%", out.height="70%", error=T, dpi=750, fig.showtext=T}
...
**NO CREO NECESARIO GRAFICAR LA DIFERENCIA ENTRE UNO Y OTRO**
...
download_data <-function(x="github"){
tryCatch({
    haven::read_dta("data/nhefs.dta")
    return(message("Already downloaded data"))
},
# ... but if an error occurs, tell me what happened: 
error=function(error_message) {
    dataurls <- list()
