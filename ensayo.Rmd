...
El nombre *“marginal structural"* apunta a estimar los efectos causales (estructurales) y marginales porque estiman el efecto en toda la población, no sólo condicional a los tratados (*average treatment effects* o *ATE* por sus siglas en inglés). 

Se distinguen 3 modelos principales:
 
- **IPWT**: El más simple es la ponderación por la probabilidad inversa de asignación a tratamiento (*inverse probability treatment weighting* o IPTW, por sus siglas en inglés).
...
Inverse probability of treatment weighting (IPTW) using the propensity score uses weights based on the propensity score to create a synthetic sample in which the distribution of measured baseline covariates is independent of treatment assignment. The use of IPTW is similar to the use of survey sampling weights that are used to weight survey samples so that they are representative of specific populations (Morgan & Todd, 2008).
 llamadas ecuaciones de estimación generalizadas (GEE). La distribución marginal de la variable respuesta es modelada como función de las covariables, asumiendo un modelo lineal generalizado o de cuasi-verosimilitud, y la asociación es incorporada para obtener estimaciones más confiables de los parámetros marginales de regresión. En este trabajo se construyen dos enfoques de modelación para el estudio de variables cuantitativas con distribución aproximadamente simétrica. - Schartz2012
...
IPW: Debilidades: ineficiente, puede ser inestable en muestras finitas por pesos grandes, no  hay propiedades para ML de propensity scores //intuitivo (ver los pesos directamente), fácil de testear y colapsar los pesos, permite manejar censura longitudinal y puede usarse directamente para MSMs más complejos
...
```{r tab-comp0_dag, eval=T,  dev.args = list(bg = 'white'), echo=T, warning=FALSE, include=T, paged.print=TRUE, fig.align="center", error=T, dpi=750, fig.showtext=T}
...
Para simplificar el ejemplo, se asume que todos cursaron el tratamiento y no hay censura, que la velocidad es constante y los riesgos son constantes y proporcionales. [@Pearce2004]
...
mod2b<- geem(dt~ t+ time+ factor(policonsumo)+ edad_ini, id=id, weights=ipw, data = datos, 
     family = MASS::negative.binomial(2), corstr = "ar1")  #El valor conocido del parámetro theta
...
Como era de esperarse, el modelo `mod1b` asumiendo una distribución Poisson tiene un mejor ajuste que el resto (asumiendo distribución binomial negativa). Una vez que se genera el modelo GEE sin ajustar por la probabilidad inversa de asignación a tratamiento, se contrasta con el modelo en el que se pondera por dicha probabilidad. 
...
- Es necesario tener en consideración que hay una tensión entre definir tan fielmente el tratamiento para cumplir SUTVA y tener la capacidad de generalizar un constructo más allá de la operacionalización de una exposición [@Schwartz2012]

### Limitaciones

- Structural Nested Models

- Confusores tiempo-modificados (*time-modified confounding*) [@platt2009]
...
- No hay una versión de métodos-g para tratamientos con tiempo continuo y resultados de supervivencia censurados.
